...
class SerializationCasesTest {
    ...
    fun testConstructorValProperties() {
        ...
        assertEquals("{a:1,b:2}", Json { unquoted = true }.stringify(data))
        assertEquals(data, Json.parse<Data1>("{a:1,b:2}"))

        // Serialize with external serializer for Data class
        assertEquals("{a:1,b:2}", Json { unquoted = true }.stringify(ExtDataSerializer1, data))
    ...
    fun testBodyVarProperties() {
        ...
        // Serialize with external serializer for Data class
        assertEquals("{a:1,b:2}", Json { unquoted = true }.stringify(ExtDataSerializer2, data))
    ...
    fun testNestedValues() {
        ...
        // Serialize with internal serializer for Data class
        assertEquals("{a:Str,b:[1,2],c:{lt:LIGHT,dk:DARK}}", Json { unquoted = true }.stringify(data))
        assertEquals(data, Json.parse<Data3>("{a:Str,b:[1,2],c:{lt:LIGHT,dk:DARK}}"))


        assertEquals("{a:Str,b:[1,2],c:{lt:LIGHT,dk:DARK}}", Json { unquoted = true }.stringify(ExtDataSerializer3, data))
